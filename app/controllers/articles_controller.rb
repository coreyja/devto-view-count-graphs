...
class ArticlesController < ApplicationController
  def index # rubocop:disable Metrics/AbcSize
    ...
    @articles_this_month = Article
                           .where(published_at: (now.beginning_of_month..now.end_of_month))
                           .order(published_at: :desc)
    previous_start = now.beginning_of_month - 4.months
    @articles_in_preview_3_months = Article
                                    .where(published_at: (previous_start...now.beginning_of_month))
                                    .order(published_at: :desc)
